% Filename:comparison4_u_modified.m
% Part 4 & 5: the effect of cost puch shock u under four different policies given modified UIP; Welfare loss

% Run the four models
dynare optimal_u_2_modified.mod noclearall nograph
% Store IRFs
optimal_u_results = oo_.irfs;

dynare Domestic_u_2a_modified.mod noclearall nograph
domestic_u_results = oo_.irfs;

dynare CPI_u_2b_modified.mod noclearall nograph
CPI_u_results = oo_.irfs;

dynare ExPeg_u_4_modified.mod noclearall nograph
ExPeg_u_results = oo_.irfs;
...
plot(periods, ExPeg_u_results.piH_eps_u, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_u_results.x_eps_u, 'k-.', 'LineWidth', 2);
title('Output Gap (x)');
xlabel('Quarters');
ylabel('Deviation from Steady State');
grid on;
...
plot(periods, ExPeg_u_results.pi_eps_u, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_u_results.tau_eps_u, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_u_results.i_eps_u, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_u_results.delta_e_eps_u, 'k-.', 'LineWidth', 2);
...
for t = 1:irf_length
    ...
    loss_optimal = Omega * 0.5 * (beta^(t-1)) * (optimal_u_results.x_eps_u(t)^2 + lambda_pi * optimal_u_results.piH_eps_u(t)^2);
    ...
    loss_domestic = Omega * 0.5 * (beta^(t-1)) * (domestic_u_results.x_eps_u(t)^2 + lambda_pi * domestic_u_results.piH_eps_u(t)^2);
    ...
    loss_CPI = Omega * 0.5 * (beta^(t-1)) * (CPI_u_results.x_eps_u(t)^2 + lambda_pi * CPI_u_results.piH_eps_u(t)^2);
    cumulative_loss_CPI(t) = cumulative_loss_CPI(max(t-1,1)) + loss_CPI;

    % Exchange rate peg policy welfare loss at time t
    loss_ExPeg = Omega * 0.5 * (beta^(t-1)) * (ExPeg_u_results.x_eps_u(t)^2 + lambda_pi * ExPeg_u_results.piH_eps_u(t)^2);
...
title('Cumulative Welfare Loss under Different Policies (u_t Shock)');
xlabel('Periods');
