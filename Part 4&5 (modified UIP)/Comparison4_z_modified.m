% Filename:Comparison4_z_modified.m
% Part 4 & 5: the effect of output-difference shock z under four different policies given modified UIP; Welfare loss

% Run the four models
dynare optimal_z_3_modified.mod noclearall nograph
% Store IRFs
optimal_z_results = oo_.irfs;

dynare Domestic_z_3a_modified.mod noclearall nograph
domestic_z_results = oo_.irfs;

dynare CPI_z_3b_modified.mod noclearall nograph
CPI_z_results = oo_.irfs;

dynare ExPeg_z_4_modified.mod noclearall nograph
ExPeg_z_results = oo_.irfs;

% Get the length of IRFs
irf_length = length(optimal_z_results.piH_eps_rn);
...
plot(periods, ExPeg_z_results.piH_eps_z, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_z_results.x_eps_z, 'k-.', 'LineWidth', 2);
title('Output Gap (x)');
xlabel('Quarters');
ylabel('Deviation from Steady State');
grid on;
...
plot(periods, ExPeg_z_results.pi_eps_z, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_z_results.tau_eps_z, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_z_results.i_eps_z, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_z_results.delta_e_eps_z, 'k-.', 'LineWidth', 2);
...
for t = 1:irf_length
    ...
    loss_optimal = Omega * 0.5 * (beta^(t-1)) * (optimal_z_results.x_eps_u(t)^2 + lambda_pi * optimal_z_results.piH_eps_u(t)^2);
    ...
    loss_domestic = Omega * 0.5 * (beta^(t-1)) * (domestic_z_results.x_eps_u(t)^2 + lambda_pi * domestic_z_results.piH_eps_u(t)^2);
    ...
    loss_CPI = Omega * 0.5 * (beta^(t-1)) * (CPI_z_results.x_eps_u(t)^2 + lambda_pi * CPI_z_results.piH_eps_u(t)^2);
    cumulative_loss_CPI(t) = cumulative_loss_CPI(max(t-1,1)) + loss_CPI;

    % Exchange rate peg policy welfare loss at time t
    loss_ExPeg = Omega * 0.5 * (beta^(t-1)) * (ExPeg_z_results.x_eps_u(t)^2 + lambda_pi * ExPeg_z_results.piH_eps_u(t)^2);
...
title('Cumulative Welfare Loss under Different Policies (z_t Shock)');
xlabel('Periods');
