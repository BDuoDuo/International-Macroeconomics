% Filename:Comparison4_istar_modified.m
% Part 4 & 5: the effect of foreign-interest-rate shock istar under four different policies given modified UIP; Welfare loss

% Run the four models
dynare optimal_istar_3_modified.mod noclearall nograph
% Store IRFs
optimal_istar_results = oo_.irfs;

dynare Domestic_istar_3a_modified.mod noclearall nograph
domestic_istar_results = oo_.irfs;

dynare CPI_istar_3b_modified.mod noclearall nograph
CPI_istar_results = oo_.irfs;

dynare ExPeg_istar_4_modified.mod noclearall nograph
ExPeg_istar_results = oo_.irfs;
...
plot(periods, ExPeg_istar_results.piH_eps_istar, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_istar_results.x_eps_istar, 'k-.', 'LineWidth', 2);
title('Output Gap (x)');
xlabel('Quarters');
ylabel('Deviation from Steady State');
grid on;
...
plot(periods, ExPeg_istar_results.pi_eps_istar, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_istar_results.tau_eps_istar, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_istar_results.i_eps_istar, 'k-.', 'LineWidth', 2);
...
plot(periods, ExPeg_istar_results.delta_e_eps_istar, 'k-.', 'LineWidth', 2);
...
for t = 1:irf_length
    ...
    loss_optimal = Omega * 0.5 * (beta^(t-1)) * (optimal_istar_results.x_eps_u(t)^2 + lambda_pi * optimal_istar_results.piH_eps_u(t)^2);
    ...
    loss_domestic = Omega * 0.5 * (beta^(t-1)) * (domestic_istar_results.x_eps_u(t)^2 + lambda_pi * domestic_istar_results.piH_eps_u(t)^2);
    ...
    loss_CPI = Omega * 0.5 * (beta^(t-1)) * (CPI_istar_results.x_eps_u(t)^2 + lambda_pi * CPI_istar_results.piH_eps_u(t)^2);
    cumulative_loss_CPI(t) = cumulative_loss_CPI(max(t-1,1)) + loss_CPI;

    % Exchange rate peg policy welfare loss at time t
    loss_ExPeg = Omega * 0.5 * (beta^(t-1)) * (ExPeg_istar_results.x_eps_u(t)^2 + lambda_pi * ExPeg_istar_results.piH_eps_u(t)^2);
...
title('Cumulative Welfare Loss under Different Policies (i^*_t Shock)');
xlabel('Periods');
